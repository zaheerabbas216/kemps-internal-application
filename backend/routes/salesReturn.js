...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const [billRows] = await connection.query(
      `SELECT customer_id, customer_name, due_amount, grand_total, amount_paid FROM customer_bills WHERE id = ? FOR UPDATE`,
      [billId]
    );
    ...
    // 4. Create Sales Return entry in approved status
    const finalStatus = settlementMethod === 'REFUND' ? 'Refunded' : 'Credit Applied';
    const parsedRefundAmount = settlementMethod === 'REFUND' ? parseFloat(refundAmount) : 0.00;

    await connection.query(
      `INSERT INTO sales_returns (
        id, return_date, customer_id, bill_id, total_return_amount, reason, created_by, cogs, 
        settlement_method, refund_amount, refund_date, refund_mode, refund_ref_no, refund_remarks, status, created_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
      [
        returnId, 
        returnDate, 
        ...
        settlementMethod,
        parsedRefundAmount,
        settlementMethod === 'REFUND' ? (refundDate || returnDate) : null,
        settlementMethod === 'REFUND' ? refundMode : null,
        settlementMethod === 'REFUND' ? refundRefNo : null,
        settlementMethod === 'REFUND' ? refundRemarks : null,
        finalStatus
      ]
    );
    ...
    // 6. Direct update of parent invoice due amount and customer credit balance
    ...
    if (totalReturnAmount <= currentDue) {
      ...
      newDue = 0;
      const excess = totalReturnAmount - currentDue;
      if (settlementMethod === 'REFUND') {
        creditBalanceAddition = Math.max(0, excess - parsedRefundAmount);
      } else {
    ...
    if (creditBalanceAddition > 0 && customerId) {
      await connection.query(
    ...
    // 7. Customer Ledger Hook: Direct ledger entries
    if (customerId) {
      ...
      if (settlementMethod === 'REFUND' && parsedRefundAmount > 0) {
        await addLedgerEntry(connection, {
          date: refundDate || returnDate,
          customerId: customerId,
          entryType: 'REFUND',
          referenceNo: returnId,
          particular: `Refund Paid — Sales Return ${returnId} (${refundMode})`,
          debit: parsedRefundAmount,
          credit: 0.00
        });
      }
    }

    // 8. If settlement is REFUND and refund amount > 0, record in expenses table for today's expenses
    if (settlementMethod === 'REFUND' && parsedRefundAmount > 0) {
      const expId = await generateId('EXP', 'expenses', 'id', connection);
      const expDate = refundDate || returnDate;
      const expParticulars = `Sales Return Refund — ${bill.customer_name || 'Customer'} (Bill ${billId})`;
      const expRemarks = `Sales Return ${returnId} refund payout. Ref: ${refundRefNo || 'N/A'}. ${refundRemarks || reason || ''}`.trim();

      await connection.query(
        `INSERT INTO expenses (
           id, expense_date, particulars, amount, entered_by, remarks, 
           payment_status, pending_amount, approved_amount, rejected_amount, approval_id, category, payment_method, created_at
         ) VALUES (?, ?, ?, ?, ?, ?, 'Approved', 0.00, ?, 0.00, NULL, 'Sales Return Refund', ?, NOW())`,
        [
          expId,
          expDate,
          expParticulars,
    ...
    res.json({
      ok: true,
      id: returnId,
      message: 'Sales return created and recorded in expenses & ledger successfully!'
    });
  } catch (error) {
  ...
  try {
    ...
    const [rows] = await pool.query(
      `SELECT 
         ...
         sr.settlement_method as settlementMethod,
         sr.refund_amount as refundAmount,
         DATE_FORMAT(sr.refund_date, '%Y-%m-%d') as refundDate,
         sr.refund_mode as refundMode,
         sr.refund_ref_no as refundRefNo,
         sr.refund_remarks as refundRemarks,
         sr.status,
         CASE WHEN sr.settlement_method = 'CREDIT' THEN sr.total_return_amount ELSE GREATEST(0.00, sr.total_return_amount - sr.refund_amount) END as creditBalanceCreated,
