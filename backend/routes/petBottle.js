...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    // Validate bottle raw material from Product Master is active
    ...
    if (prodCheck.length > 0 && prodCheck[0].status === 0) {
      throw new Error(`The bottle product '${prodCheck[0].name}' is disabled in Product Master. You cannot place new production entries for it.`);
    }
    ...
    // 3. Log produced bottles to stock register (Positive addition for raw materials - Bottles category)
    await connection.query(
      `INSERT INTO stock_register (item_type, item_id, transaction_type, reference_id, quantity, created_at)
       VALUES ('RAW_MATERIAL', ?, 'PRODUCTION', ?, ?, NOW())`,
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    if (prodCheck.length > 0 && prodCheck[0].status === 0) {
      throw new Error(`The product '${prodCheck[0].name}' is disabled in Product Master. You cannot place new production entries for it.`);
    }
