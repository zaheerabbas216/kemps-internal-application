...
async function syncOrderAdvancePayment(connection, { orderId, customerId, advanceAmount, paymentMode, orderCompany = 'Kempannavar Industries' }) {
  ...
  if (existingPay.length > 0) {
    for (const p of existingPay) {
      await deleteLedgerEntriesForReference(connection, p.id);
      await connection.query(`DELETE FROM customer_payments WHERE id = ?`, [p.id]);
    }
  }
  ...
  if (advVal > 0) {
    ...
    const pMode = String(paymentMode || 'Cash').trim();

    const cashAmt = pMode.toLowerCase().includes('cash') ? advVal : 0;
    const upiAmt  = pMode.toLowerCase().includes('upi')  ? advVal : 0;
    const bankAmt = (!pMode.toLowerCase().includes('cash') && !pMode.toLowerCase().includes('upi')) ? advVal : 0;

    await connection.query(
      `INSERT INTO customer_payments 
       (id, bill_id, company, customer_id, payment_date, payment_method, amount_received, cash_paid, upi_paid, bank_paid, remarks, 
      ...
      [
        ...
        upiAmt,
        bankAmt,
        `Advance Payment for Order #${orderId}`,
        advVal
      ]
    );
    ...
    await addLedgerEntry(connection, {
      ...
      particular: `Advance Payment Received for Order #${orderId} (${pMode})`,
      debit: 0.00,
      credit: advVal
    });
  }
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    // 5. If advance payment received, record as Customer Credit Balance in Accounts Ledger
...
router.post('/:id/cancel', async (req, res) => {
  try {
    ...
    // Clean up advance payment ledger entries if order is cancelled
    try {
      ...
      for (const p of existingPay) {
        await deleteLedgerEntriesForReference(pool, p.id);
        await pool.query(`DELETE FROM customer_payments WHERE id = ?`, [p.id]);
      }
    } catch (e) {
      console.error('Error cleaning up advance payment on order cancel:', e);
    }
