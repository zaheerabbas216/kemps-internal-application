...
router.post('/manual-bill', async (req, res) => {
  ...
  try {
    ...
    const { billDate, supplierId, billedTo, billNumber, grandTotal, advancePaid = 0, creditNote = 0, remarks, description } = req.body;
    ...
    await connection.query(
      `INSERT INTO inventory_bills 
       (id, bill_date, supplier_id, billed_to, bill_number, payment_method, sub_total, total_tax, additional_expenses, grand_total, remarks, description, is_manual, advance_paid, credit_note, status, created_at)
...
router.post('/pay', async (req, res) => {
  ...
  try {
    ...
    // 2. Fetch sum of already approved instalments
    const [payRows] = await connection.query(
      `SELECT 
    ...
    res.json({ ok: true, id: payId, message: 'Payment recorded successfully.' });
  } catch (error) {
    await connection.rollback();
