...
router.get('/users', authMiddleware, async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT id, username, name, password, created_at FROM admins ORDER BY username ASC');
...
router.post('/users', authMiddleware, async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      ...
      [usernameTrimmed, password, nameTrimmed]
    );
...
router.put('/users/:id', authMiddleware, async (req, res) => {
  try {
    ...
    if (password) {
      // Update with password change (plain text)
      await pool.query(
        'UPDATE admins SET username = ?, password = ?, name = ? WHERE id = ?',
        [usernameTrimmed, password, nameTrimmed, id]
      );
    } else {
