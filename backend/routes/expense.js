...
// GET /api/expenses/consolidated
// Returns expenses grouped by particulars with total amount & count, supports search + date filters
router.get('/consolidated', async (req, res) => {
  try {
    ...
    if (search.trim()) {
      whereClauses.push('(particulars LIKE ? OR category LIKE ?)');
      const wildSearch = `%${search.trim()}%`;
    ...
    res.json({ ok: true, consolidated: rows, grandTotal });
  } catch (error) {
    res.status(400).json({ ok: false, error: error.message });
  }
});
...
router.post('/', async (req, res) => {
  try {
    ...
    await pool.query(
      ...
      [id, expenseDate, particularsTrimmed, amountVal, enteredByTrimmed, String(remarks || '').trim(), amountVal, categoryVal, paymentMethodVal]
    );
