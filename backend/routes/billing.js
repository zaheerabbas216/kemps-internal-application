...
// GET /api/billing/dashboard-history - Retrieves historical daily summary of sales and expenses
router.get('/dashboard-history', async (req, res) => {
  try {
    ...
    // Query sales aggregates per day
    ...
    // Query expenses aggregates per day
    const [expensesRows] = await pool.query(
      `SELECT 
         DATE_FORMAT(expense_date, '%Y-%m-%d') AS date,
         COALESCE(SUM(CASE WHEN payment_status = 'Approved' THEN amount ELSE 0 END), 0) AS totalExpenses
       FROM expenses
    ...
    const getOrCreateEntry = (dateStr) => {
    ...
    salesRows.forEach(row => {
      ...
      entry.totalSales = parseFloat(row.totalSales) || 0;
    });

    expensesRows.forEach(row => {
      const entry = getOrCreateEntry(row.date);
      entry.totalExpenses = parseFloat(row.totalExpenses) || 0;
    });

    const historyList = Object.values(historyMap).map(entry => {
      entry.netSales = entry.totalSales - entry.totalExpenses;
      return entry;
    });
...
router.get('/today', async (req, res) => {
  try {
    ...
    // Get today's aggregates — cashCollected = cash+UPI+bank only (NOT credit applied)
    const [summaryRows] = await pool.query(
      `SELECT 
         COALESCE(SUM(grand_total), 0)                              AS totalBilled,
         COALESCE(SUM(cash_paid + upi_paid + bank_paid), 0)        AS cashCollected,
         COALESCE(SUM(due_amount), 0)                              AS creditDue,
         COALESCE(SUM(amount_paid - (cash_paid + upi_paid + bank_paid)), 0) AS creditApplied,
    ...
    const summary = {
      ...
      creditApplied: parseFloat(summaryRows[0].creditApplied),
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (String(paymentMode || '').trim().toLowerCase() === 'credit') {
      finalCashPaid = 0.00;
      finalUpiPaid = 0.00;
      finalBankPaid = 0.00;
    }
    ...
    if (applyCredit === true && customerCreditBalance > 0.00 && finalDueAmount > 0.00) {
    ...
    // Auto-create Payment Approval entry ONLY for cash/UPI/bank portions
    ...
    // 3. Insert Invoice Header directly with approved status & amounts
    await connection.query(
      `INSERT INTO customer_bills 
       (id, billing_date, company, customer_type, customer_id, customer_name, customer_phone, customer_gstin, customer_address, 
        ...
        payment_status, pending_amount, approved_amount, rejected_amount, approval_id, cogs, profit, remarks, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0.00, ?, 0.00, NULL, ?, ?, ?, NOW())`,
      [
        billId,
        billingDate,
        company,
    ...
    // Customer Ledger Hook: SALE debit for invoice grand total
    ...
    // Customer Ledger Hook: Immediate PAYMENT credit for amount paid (excluding any applied credit balance)
    const actualNewPayment = finalAmountPaid - (parseFloat(appliedCredit) || 0.00);
    if (actualNewPayment > 0) {
      await addLedgerEntry(connection, {
        date: billingDate,
        customerId: customerId,
        entryType: 'PAYMENT',
        referenceNo: billId,
        particular: `Payment received for Invoice ${billId} (${combinedMode})`,
        debit: 0.00,
        credit: actualNewPayment
      });
    }

    // No extra ledger entry is needed for applied credit balance since the debit invoice and pre-existing credit entries already offset each other.
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    // Customer Ledger Hook: Clear old entries and insert updated SALE debit and PAYMENT credit
    ...
    if (finalAmountPaid > 0) {
      await addLedgerEntry(connection, {
        ...
        entryType: 'PAYMENT',
        referenceNo: id,
        particular: `Payment received for Invoice ${id} (${finalPaymentMode})`,
        debit: 0.00,
