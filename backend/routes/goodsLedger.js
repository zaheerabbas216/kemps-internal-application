...
async function calculateDynamicRow(connection, product, targetDate) {
  ...
  if (hasManual && hasClosed) {
    ...
    if (dManual > dClosed) {
      ...
      startQueryDate = dManual; // include manual date transactions in dynamic sum
    } else {
  ...
  // 3. Get transactions up to targetDate - 1 to calculate opening stock of targetDate
  ...
  if (!startQueryDate || startQueryDate <= yesterdayStr) {
    ...
    if (startQueryDate) {
      const [pRow] = await connection.query(
        ...
        [product.id, startQueryDate, yesterdayStr]
      );
    ...
    openingStock = checkpointStock + prodIn - loadOut;
  }

  // 4. Get transactions on the targetDate itself
  const [pTodayRow] = await connection.query(
    `SELECT COALESCE(SUM(production_boxes), 0) AS count FROM production_batches 
     WHERE finished_product_id = ? AND production_date = ?`,
    [product.id, targetDate]
  );
  const stockIn = parseFloat(pTodayRow[0].count) || 0;

  const [lTodayRow] = await connection.query(
    `SELECT COALESCE(SUM(lti.quantity), 0) AS count FROM loading_trip_items lti
     JOIN loading_trips lt ON lti.trip_id = lt.id
     JOIN loading_sessions ls ON lt.session_id = ls.id
     WHERE lti.finished_product_id = ? AND ls.loading_date = ?`,
  ...
  const stockOut = parseFloat(lTodayRow[0].count) || 0;
    ...
    for (const prod of products) {
      ...
      await connection.query(
        `INSERT INTO finished_goods_ledger_snapshots 
           (ledger_date, finished_product_id, opening_stock, stock_in, stock_out, closing_stock)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [
          date,
  ...
  try {
    ...
    if (type === 'IN') {
      ...
      if (!startQueryDate || startQueryDate <= yesterdayStr) {
        ...
        transactions = transactions.concat(priorProd).concat(priorLoad);
      }
