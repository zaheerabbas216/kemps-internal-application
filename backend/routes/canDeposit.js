...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (paymentMode === 'Cash') cashPaid = amountVal;
    else if (paymentMode === 'UPI') upiPaid = amountVal;
    else if (paymentMode === 'Bank Transfer') bankPaid = amountVal;
    ...
    await connection.query(
      ...
      [
        billId,
        billingDate,
        customer.customer_type || 'General Customer',
        customerId,
        customer.name,
        customer.phone,
        customer.gstin || '',
    ...
    // 7. Insert bill item details
    await connection.query(
      `INSERT INTO customer_bill_items 
       (bill_id, finished_product_id, quantity, rate_with_tax, tax_percent, basic_rate, total_amount)
       VALUES (?, ?, ?, ?, 0.00, ?, ?)`,
      [billId, canDepositProductId, qtyVal, rateVal, rateVal, amountVal]
    );

    // 8. Log deduction in stock register
    await connection.query(
      ...
      [canDepositProductId, billId, -qtyVal]
    );
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    // 2. Remove associated accounting entry
    if (transaction.transaction_type === 'Deposit Received') {
      // Find corresponding billing invoice
      // Since it links to 'Can Deposit' finished product, we lookup the bill
      const [prodRows] = await connection.query(
      ...
      if (prodRows.length > 0) {
        ...
        // Find bill ID matching conditions
        const [billRows] = await connection.query(
          `SELECT b.id FROM customer_bills b
           JOIN customer_bill_items i ON b.id = i.bill_id
           WHERE b.customer_id = ? 
             AND b.grand_total = ? 
             AND i.finished_product_id = ? 
             AND DATE(b.created_at) = DATE(?)
           LIMIT 1`,
          [customerId, transaction.amount, canDepositProductId, transaction.created_at]
        );

        if (billRows.length > 0) {
          const billId = billRows[0].id;

          // Delete bill, item details, and stock register entries
          await connection.query(`DELETE FROM stock_register WHERE reference_id = ?`, [billId]);
          await connection.query(`DELETE FROM customer_bill_items WHERE bill_id = ?`, [billId]);
          await connection.query(`DELETE FROM customer_bills WHERE id = ?`, [billId]);
        }
      }
    } else if (transaction.transaction_type === 'Deposit Returned') {
