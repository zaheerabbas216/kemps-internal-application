...
router.get('/history', async (req, res) => {
  try {
    let { page = 1, limit = 10, search = '', startDate = '', endDate = '', godown = '', status = '', statusNot = '' } = req.query;
    ...
    if (status) {
      whereClauses.push('ls.status = ?');
      queryParams.push(status);
    }
    if (statusNot) {
      whereClauses.push('ls.status != ?');
      queryParams.push(statusNot);
    }
    if (search.trim()) {
...
// POST /api/loading/session/:id/complete - Mark loading session as completed
router.post('/session/:id/complete', async (req, res) => {
  try {
    ...
    if (sessionRows[0].status !== 'ACTIVE') {
      return res.status(400).json({ ok: false, error: `Loading session is already in '${sessionRows[0].status}' status.` });
    }

    await pool.query(
      "UPDATE loading_sessions SET status = 'COMPLETED' WHERE id = ?",
      [id]
    );

    res.json({ ok: true, message: 'Loading session completed successfully.' });
  } catch (error) {
    res.status(400).json({ ok: false, error: error.message });
  }
});

// POST /api/loading/session/:id/return - Record active loading returns (supports multiple items)
router.post('/session/:id/return', async (req, res) => {
  ...
  try {
    ...
    if (!items && req.body.finishedProductId) {
      items = [{
        finishedProductId: req.body.finishedProductId,
    ...
    // 3. Create a single return trip in loading_trips
    const tripId = await generateId('TRIP', 'loading_trips', 'id', connection);
    const [tripCount] = await connection.query(
      `SELECT COUNT(*) as count FROM loading_trips WHERE session_id = ?`,
      [id]
    ...
    const consolidatedReasons = items
      .map(it => String(it.reason || 'Loading Return').trim())
      .filter((val, idx, self) => val && self.indexOf(val) === idx)
      .join(', ');
    const tripRemarks = `Active Return: ${consolidatedReasons || 'Items Return'}`;

    await connection.query(
      `INSERT INTO loading_trips (id, session_id, trip_number, godown, remarks, created_at)
       VALUES (?, ?, ?, ?, ?, NOW())`,
      [tripId, id, tripNumber, godown, tripRemarks.substring(0, 255)]
    );

    // 4. Process each return item
    for (const item of items) {
      const { finishedProductId, quantity, reason } = item;
      
      if (!finishedProductId) throw new Error('Product selection is required for all items.');
      const q = parseInt(quantity, 10);
      if (isNaN(q) || q <= 0) throw new Error('Return quantity must be greater than 0.');
      const r = String(reason || '').trim() || 'Loading Return';

      // Fetch the product details and verify it's loaded in this session
      ...
      if (totalLoaded === 0) {
        const [prodNameRows] = await connection.query(
          `SELECT name FROM finished_products WHERE id = ?`,
          [parseInt(finishedProductId, 10)]
        );
        const prodName = prodNameRows[0]?.name || `ID ${finishedProductId}`;
        throw new Error(`Product "${prodName}" is not loaded in this session.`);
      }
      if (q > availableToReturn) {
        ...
        const prodName = prodNameRows[0]?.name || `ID ${finishedProductId}`;
        throw new Error(`Return quantity (${q}) for "${prodName}" exceeds the available loaded quantity (${availableToReturn}).`);
      }

      // Record return transaction in loading_returns for audit trail
      await connection.query(
        `INSERT INTO loading_returns (session_id, finished_product_id, quantity, reason, user_name, created_at)
         VALUES (?, ?, ?, ?, ?, NOW())`,
        [id, parseInt(finishedProductId, 10), q, r, userName]
      );

      // Insert trip item in loading_trip_items with quantity = -q and return_qty = 0
      await connection.query(
        `INSERT INTO loading_trip_items (trip_id, finished_product_id, quantity, return_qty, created_at)
         VALUES (?, ?, ?, 0, NOW())`,
