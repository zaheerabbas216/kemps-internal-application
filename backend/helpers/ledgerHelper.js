...
export async function deleteLedgerEntriesForReference(connection, referenceNo) {
  // FIX: Collect ALL distinct customer_ids for this reference — not just the first one.
  // Previously only the first customer_id was used for balance recalculation,
  // which could leave other customers with stale balances (payment history bug).
  const [rows] = await connection.query(
  ...
  if (rows.length > 0) {
    ...
    // Recalculate balances for every affected customer
    for (const row of rows) {
      await recalculateLedgerBalances(connection, row.customer_id);
    }
  }
}
...
export async function recalculateLedgerBalances(connection, customerId) {
  ...
  // Sync credit_balance column in customers table
  const finalCreditBalance = balance < 0 ? Math.abs(balance) : 0.00;
  await connection.query(
    `UPDATE customers SET credit_balance = ? WHERE id = ?`,
    [finalCreditBalance, customerId]
  );
}
