...
const Layout = () => {
  ...
  // Fetch pending payment approval count from API
  useEffect(() => {
        ...
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200/50 flex items-center justify-between px-4 sm:px-6 md:px-10 sticky top-0 z-20">
          ...
          <div className="flex items-center gap-2 lg:hidden">
            <div 
              ...
              className="relative w-10 h-10 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-center text-slate-600 cursor-pointer hover:bg-slate-100 transition-all active:scale-95 animate-fade-in"
              title="Payment Approvals"
            >
              <span className="text-lg">🔔</span>
              {pendingCount > 0 && (
                <span className="absolute -top-1.5 -right-1.5 min-w-5 h-5 px-1.5 rounded-full bg-amber-500 text-white text-[10px] font-black flex items-center justify-center border-2 border-white shadow-sm">
                  {pendingCount}
                </span>
              )}
            </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-white/95 backdrop-blur-md border-t border-slate-200/60 z-30 flex items-center justify-around px-2 shadow-[0_-4px_12px_rgba(0,0,0,0.03)] pb-safe">
        ...
        >
          <span className="text-xl relative">
            💰
            {pendingCount > 0 && (
              <span className="absolute -top-1.5 -right-2 min-w-4 h-4 px-1 rounded-full bg-amber-500 text-white text-[8px] font-black flex items-center justify-center border border-white">
                {pendingCount}
              </span>
            )}
          </span>
