...
const ExpenseHistory = () => {
  ...
  // View mode: 'history' | 'consolidated'
  ...
  }, [currentPage, searchQuery, startDate, endDate, activeTab]);

  const fetchConsolidated = async () => {
    try {
      setLoadingConsolidated(true);
      const res = await api.get('/expenses/consolidated', {
      ...
      console.error('Failed to fetch consolidated expenses:', err);
    } finally {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex gap-1 p-1 bg-slate-100 rounded-xl w-fit">
        ...
        <button
          onClick={() => { setActiveTab('consolidated'); setCurrentPage(1); }}
          className={`px-5 py-2 rounded-lg text-[13px] font-bold transition-all ${
        ...
        <div className="flex justify-end mb-4">
          {activeTab === 'history' ? (
            ...
            <div className="flex items-center gap-3">
              <div className="text-[12px] font-bold text-slate-400 uppercase tracking-widest bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100">
                {consolidated.length} Unique Particulars
              </div>
              <div className="text-[12px] font-bold text-primary bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-100">
            ...
            <table className="table table-zebra w-full overflow-hidden">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr className="text-slate-500 text-[11px] font-black uppercase tracking-wider">
                  ...
                  <th className="py-4 px-6 text-left">Particulars</th>
                  <th className="py-4 px-6 text-left">Category</th>
                  <th className="py-4 px-6 text-center">Entries</th>
                  <th className="py-4 px-6 text-left">First Entry</th>
                  <th className="py-4 px-6 text-left">Last Entry</th>
                ...
                {loadingConsolidated ? (
                  <tr>
                    <td colSpan="7" className="py-20 text-center">
                      <div className="flex flex-col items-center gap-3">
                        <span className="loading loading-spinner text-primary"></span>
                        <span className="text-slate-400 text-sm font-medium">Building consolidated report...</span>
                      </div>
                    </td>
                  </tr>
                  ...
                  <>
                    {consolidated.map((row, idx) => (
                      <tr
                        key={`${row.particulars}-${row.category}`}
                        className="hover:bg-blue-50/30 transition-colors cursor-pointer group"
                        onClick={() => {
                          setSearchQuery(row.particulars);
                          setActiveTab('history');
                          setCurrentPage(1);
                        }}
                        title="Click to view individual entries for this particular"
                      >
                        <td className="py-4 px-6 text-[12px] font-bold text-slate-400">{idx + 1}</td>
                        <td className="py-4 px-6 text-[14px] font-bold text-slate-800 max-w-[280px]">
                          <div className="flex items-center gap-2">
                            <span className="truncate">{row.particulars}</span>
                            <span className="text-[10px] text-slate-400 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">→ view entries</span>
                          </div>
                        </td>
                        <td className="py-4 px-6">
                          <span className="text-[11px] bg-slate-100 text-slate-600 px-2.5 py-1 rounded-lg font-bold">
                            {row.category || 'General'}
                          </span>
                        </td>
                        <td className="py-4 px-6 text-center">
                          <span className="text-[13px] font-black text-slate-700 bg-slate-50 border border-slate-200 rounded-full w-8 h-8 inline-flex items-center justify-center">
                            {row.entry_count}
                          </span>
                        </td>
                        <td className="py-4 px-6 text-[13px] font-medium text-slate-500">
                          {formatDateDDMMYYYY(row.first_date)}
                        </td>
                        <td className="py-4 px-6 text-[13px] font-medium text-slate-500">
                          {formatDateDDMMYYYY(row.last_date)}
                        </td>
                        <td className="py-4 px-6 text-right text-[15px] font-black text-red-600">
                    ...
                    <tr className="bg-slate-800 text-white">
                      <td colSpan="6" className="py-4 px-6 text-[12px] font-black uppercase tracking-widest text-slate-300">
                        Grand Total ({consolidated.reduce((s, r) => s + parseInt(r.entry_count), 0)} entries across {consolidated.length} particulars)
                      </td>
                      <td className="py-4 px-6 text-right text-[16px] font-black text-white">
