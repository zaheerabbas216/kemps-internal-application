...
const MODULE_LABELS = {
  ...
  CanSupplyPayment: { label: 'Can Supply Payment', icon: '💧', color: 'bg-indigo-50 text-indigo-700 border-indigo-200' },
  SalesReturn: { label: 'Sales Return', icon: '🔄', color: 'bg-amber-50 text-amber-700 border-amber-200' },
};
  ...
  return (
    <div className={`bg-white rounded-2xl border shadow-sm overflow-hidden transition-all duration-200 hover:shadow-md ${
      ...
      <div className="px-5 py-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-3 text-xs">
        ...
        {(parseFloat(entry.cash_amount) > 0 || parseFloat(entry.upi_amount) > 0 || parseFloat(entry.bank_amount) > 0 || parseFloat(entry.credit_amount) > 0) && (
          <div className="col-span-2 md:col-span-1">
            ...
            <div className="flex flex-wrap gap-1.5 mt-0.5">
              ...
              {parseFloat(entry.credit_amount) > 0 && (
                <span className="px-1.5 py-0.5 bg-orange-50 border border-orange-300 rounded text-[10px] font-bold text-orange-600">
                  💳 Credit Applied: {fmt(entry.credit_amount)}
                </span>
              )}
            </div>
          </div>
      ...
      <div className="px-5 py-3 bg-slate-50/60 border-t border-slate-100 flex items-center justify-between gap-4">
        <div>
          {parseFloat(entry.amount) > 0 && (
            <>
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Cash / UPI / Bank</p>
          ...
          {parseFloat(entry.credit_amount) > 0 && (
            <>
              <p className="text-[9px] font-black text-orange-400 uppercase tracking-widest mt-1">Credit Applied</p>
              <p className="text-xl font-black text-orange-500 mt-0.5">
                💳 {fmt(entry.credit_amount)}
              </p>
            </>
          )}
          {parseFloat(entry.amount) === 0 && parseFloat(entry.credit_amount) === 0 && (
            <>
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Total Amount</p>
              <p className="text-xl font-black text-slate-400 mt-0.5">₹0.00</p>
            </>
          )}
        </div>
        <div className="flex items-center gap-2">
