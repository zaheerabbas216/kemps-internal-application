...
const GoodsLedger = () => {
  ...
  const fetchLedger = async () => {
    ...
    try {
      setLoading(true);
      const res = await api.get('/goods-ledger/day', {
      ...
      console.error('Failed to fetch finished goods ledger:', err);
  ...
  const handleStartEditOpening = () => {
    ...
    (ledgerData.items || []).forEach(item => {
      initialStocks[item.finished_product_id] = item.opening_stock ?? 0;
    });
  ...
  const handleStockChange = (item, value) => {
    setOpeningStocks(prev => ({
      ...prev,
      [item.finished_product_id]: value
    }));
  };
  ...
  const handleSaveOpening = async (e) => {
    ...
    try {
      const items = (ledgerData.items || []).map(item => {
        const val = openingStocks[item.finished_product_id];
        return {
      ...
      const res = await api.post('/goods-ledger/set-opening', {
        date: ledgerDate,
        items
      });
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          ...
          <div className="md:col-span-1 space-y-1.5">
            ...
            <input
              ...
              className="w-full h-11 px-4 rounded-xl border border-slate-200 bg-white text-slate-755 focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all outline-none text-sm font-semibold"
            />
          </div>
        ...
        <div className="card-premium border-2 border-amber-200 bg-[#fffdf6] p-6 space-y-6 shadow-md rounded-3xl animate-fade-in">
          <div className="border-b border-amber-100 pb-4">
            ...
            <p className="text-amber-700/80 text-xs font-semibold mt-1">
              Enter opening stock for each finished product. Leave 0 if not holding that item.
            </p>
          </div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
            ...
            <div className="card-premium flex items-center justify-between p-5">
              <div>
                ...
                <h3 className="text-lg font-extrabold text-emerald-600 mt-1">
                  +{(ledgerData.summary?.totalStockInToday || 0).toLocaleString('en-IN')} Boxes
                </h3>
              </div>
            ...
            <div className="card-premium flex items-center justify-between p-5">
              <div>
                <p className="text-[10px] font-black text-slate-450 uppercase tracking-widest">
                  Total Stock OUT (Load)
                </p>
                <h3 className="text-lg font-extrabold text-rose-500 mt-1">
                  -{(ledgerData.summary?.totalStockOutToday || 0).toLocaleString('en-IN')} Boxes
                </h3>
              </div>
            ...
            <div className="card-premium flex items-center justify-between p-5">
              <div>
                ...
                <h3 className="text-lg font-extrabold text-amber-600 mt-1">
                  +{(ledgerData.summary?.totalStockReturnToday || 0).toLocaleString('en-IN')} Boxes
                </h3>
              </div>
            ...
            <div className="card-premium flex items-center justify-between p-5">
              <div>
                ...
                <h3 className="text-lg font-extrabold text-[#0066cc] mt-1">
                  {(ledgerData.summary?.totalClosingStock || 0).toLocaleString('en-IN')} Boxes
                </h3>
              </div>
              <div className="text-xl">🏦</div>
            </div>
          ...
          {loading ? (
            <div className="card-premium py-20 text-center text-slate-400">
              ...
              Calculating finished goods stocks for {formatDateDDMMYYYY(ledgerDate)}...
            </div>
          ) : Object.keys(groupedItems).length === 0 ? (
            <div className="card-premium py-20 text-center text-slate-400 italic">
              No finished products found. Add finished products in the Product Master first.
            </div>
          ) : (
                      ...
                      <table className="table w-full">
                        <thead>
                          <tr className="bg-slate-50/50 text-slate-500 text-[11px] font-extrabold uppercase tracking-wider border-b border-slate-100">
                            <th className="py-3 px-5 text-left w-1/3">Finished Product</th>
                            <th className="py-3 px-5 text-center w-1/12">Unit</th>
                            <th className="py-3 px-5 text-right">Opening</th>
                            <th className="py-3 px-5 text-right text-emerald-600">+IN (Prod)</th>
                            <th className="py-3 px-5 text-right text-rose-500">-OUT (Load)</th>
                            <th className="py-3 px-5 text-right text-amber-600">+RETURN</th>
                        ...
                        <tfoot>
                          <tr className="bg-[#f8fafc] border-t border-slate-200/80 text-slate-700 font-extrabold text-xs">
                            ...
                            <td className="py-3.5 px-5 text-right text-amber-700 font-black">
          ...
          {!loading && ledgerData.items?.length > 0 && (
            <div className="bg-[#0b1329] text-white py-4 px-6 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-lg animate-fade-in">
              ...
              <div className="flex flex-wrap gap-8 text-right justify-end text-xs md:text-sm">
                <div>
                  <p className="text-[9px] font-black text-slate-444 uppercase tracking-widest">Opening</p>
                  <h4 className="font-black mt-0.5">{grandTotals.opening.toLocaleString('en-IN')}</h4>
                </div>
                <div>
                  <p className="text-[9px] font-black text-slate-444 uppercase tracking-widest">Stock IN</p>
                  <h4 className="font-black text-emerald-400 mt-0.5">+{grandTotals.stockIn.toLocaleString('en-IN')}</h4>
                </div>
                <div>
                  <p className="text-[9px] font-black text-slate-444 uppercase tracking-widest">Stock OUT</p>
                  <h4 className="font-black text-rose-455 mt-0.5">-{grandTotals.stockOut.toLocaleString('en-IN')}</h4>
                </div>
                <div>
                  <p className="text-[9px] font-black text-slate-444 uppercase tracking-widest">Return</p>
                  <h4 className="font-black text-amber-400 mt-0.5">+{grandTotals.stockReturn.toLocaleString('en-IN')}</h4>
                </div>
                <div>
                  <p className="text-[9px] font-black text-slate-444 uppercase tracking-widest">Closing</p>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm pointer-events-auto">
          <div className="bg-white rounded-3xl shadow-[0_25px_80px_rgba(0,0,0,0.2)] border border-slate-200 w-[750px] max-h-[90vh] h-[520px] p-8 flex flex-col overflow-hidden animate-fade-in pointer-events-auto relative">
            ...
            <h3 className="text-xl font-heading font-black text-slate-800 mb-1 uppercase tracking-tight">
              🔍 Finished Goods Stock Movement Audit Trail
            </h3>
            <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-6">
              {drillData.categoryName} — {drillData.productName} | Type: {drillData.type} on {formatDateDDMMYYYY(ledgerDate)}
            </p>
