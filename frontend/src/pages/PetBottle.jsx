  ...
  const fetchDropdowns = async () => {
    try {
      ...
      if (materialsRes.data.ok) {
        ...
        // 1. Filter Raw Materials under "Bottles" category
        const bottleCat = allCats.find(c => c.name.toLowerCase() === 'bottles');
        let bottleMats = [];
        if (bottleCat) {
          bottleMats = allMats.filter(m => m.category_id === bottleCat.id);
        } else {
          bottleMats = allMats.filter(m => m.category_name && m.category_name.toLowerCase() === 'bottles');
        }
        setBottleProducts(bottleMats);

        // 2. Filter Raw Materials under "Preforms" category
        const preformCat = allCats.find(c => c.name.toLowerCase() === 'preforms');
        let preformMats = [];
        if (preformCat) {
          preformMats = allMats.filter(m => m.category_id === preformCat.id);
        } else {
          preformMats = allMats.filter(m => m.category_name && m.category_name.toLowerCase() === 'preforms');
        }
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      >
                        ...
                        {bottleProducts.map(bp => (
                          <option key={bp.id} value={bp.id}>{bp.sub_product_name}</option>
                        ))}
                      </select>
                    </div>
