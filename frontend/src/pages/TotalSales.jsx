...
function KpiCard({ icon, label, value, sub, color, bg, trend }) {
  ...
  const hasRupee = valueStr.startsWith('₹');
  const cleanValue = hasRupee ? valueStr.slice(1) : valueStr;

  return (
    <div className={`rounded-2xl border border-white shadow-sm p-5 ${bg} relative overflow-hidden flex flex-col items-center text-center`}>
      <div className="flex flex-col items-center justify-center mb-3">
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-xl`} style={{ background: color + '22' }}>
          {icon}
        </div>
        {trend !== undefined && (
          <span className={`text-[10px] font-black px-2 py-1 rounded-full mt-2 ${trend >= 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-600'}`}>
            {trend >= 0 ? '↑' : '↓'} {Math.abs(trend)}%
          </span>
        )}
      </div>
      <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">{label}</p>
      <h3 className="text-2xl font-black leading-tight flex items-center justify-center gap-1" style={{ color }}>
        {hasRupee && <span>₹</span>}
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm font-medium mt-1">
            Complete sales analytics — Invoices, Collections and Can Supply data
          </p>
        </div>
      ...
      {loadingKpi ? <Spinner /> : kpi && (
        <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-3 md:gap-4">
          ...
          <KpiCard
            ...
            value={formatINR(kpi.totalExpenses || 0)}
          ...
          <KpiCard
            ...
            value={formatINR(kpi.supplierPayments || 0)}
          ...
          <KpiCard
            icon="📈"
            label="Net Revenue"
            value={formatINR((kpi.totalCollection || 0) - (kpi.totalExpenses || 0))}
          ...
          <div className="p-6 space-y-6">
            ...
            {loadingTrend ? <Spinner /> : (
              <>
                ...
                {monthlyTrend.some(m => (parseFloat(m.totalExpenses) || 0) > 0) && (
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div className="col-span-2 md:col-span-1 bg-rose-50 rounded-2xl border border-rose-100 shadow-sm p-4 flex items-center gap-4">
                      <div className="w-10 h-10 bg-rose-100 rounded-xl flex items-center justify-center text-xl shrink-0">💸</div>
                      <div>
                        <p className="text-[10px] font-black uppercase tracking-widest text-rose-600/70">Total Expenses</p>
                        <p className="text-xl font-black text-rose-600">
                          {formatINR(monthlyTrend.reduce((s, m) => s + (parseFloat(m.totalExpenses || 0) || 0), 0))}
                        </p>
                        <p className="text-[10px] text-rose-400 font-medium mt-0.5">Approved expenses · {year}</p>
                      </div>
                    </div>
                    <div className="col-span-2 md:col-span-1 bg-slate-50 rounded-2xl border border-slate-100 shadow-sm p-4 flex items-center gap-4">
                      <div className="w-10 h-10 bg-slate-200 rounded-xl flex items-center justify-center text-xl shrink-0">📊</div>
                      <div>
                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Net Revenue</p>
                        <p className="text-xl font-black text-slate-700">
                          {formatINR(
                            monthlyTrend.reduce((s, m) => s + (parseFloat(m.totalSales) || 0), 0) -
                            monthlyTrend.reduce((s, m) => s + (parseFloat(m.totalExpenses || 0) || 0), 0)
                          )}
                        </p>
                        <p className="text-[10px] text-slate-400 font-medium mt-0.5">Sales − Expenses · {year}</p>
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-slate-50">
                      {monthlyTrend.map((m, i) => (
                        <tr key={i} className={`hover:bg-blue-50/30 transition-colors ${parseFloat(m.totalSales) === 0 ? 'opacity-40' : ''}`}>
                          ...
                          <td className="py-3 px-4 text-right font-bold text-rose-500">{formatINRFull(m.totalExpenses || 0)}</td>
                    ...
                    <tfoot className="bg-slate-50 border-t border-slate-200">
                      <tr className="font-black text-slate-800">
                        ...
                        <td className="py-3 px-4 text-right text-rose-500">{formatINRFull(monthlyTrend.reduce((s, m) => s + (parseFloat(m.totalExpenses||0)||0), 0))}</td>
