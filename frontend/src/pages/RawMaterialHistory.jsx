...
const RawMaterialHistory = () => {
  ...
  const fetchHistory = async () => {
    setLoading(true);
    try {
      const res = await api.get('/raw-material-ledger/history', {
        params: {
          page: currentPage,
          limit,
          startDate,
      ...
      console.error('Failed to load raw material history:', err);
      alert(err.response?.data?.error || 'Failed to load raw material history.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6 animate-fade-in max-w-5xl mx-auto pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <button
            onClick={() => navigate('/raw-material-ledger')}
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-800 tracking-tight uppercase">
              RAW MATERIAL CLOSING HISTORY
            </h1>
            <p className="text-slate-500 text-xs font-semibold mt-0.5">
              Permanent archive of all automatically closed daily ledgers and snapshots
            </p>
          </div>
        </div>
        <div>
              ...
              >
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-4">
                  ...
                  <div className="flex flex-wrap gap-4 text-xs font-bold">
                    <div className="bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-100">
                      <span className="text-slate-400 uppercase text-[9px] block">Opening Value</span>
                      <span className="text-slate-800 font-black">
                        ₹{(dayRecord.summary?.totalRawMaterialValue || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                      </span>
                    </div>
                    <div className="bg-emerald-50/60 px-3 py-1.5 rounded-xl border border-emerald-100">
                      <span className="text-emerald-600 uppercase text-[9px] block">Stock IN</span>
                      <span className="text-emerald-700 font-black">
                        +{(dayRecord.summary?.totalStockInToday || 0).toLocaleString('en-IN')}
                      </span>
                    </div>
                    <div className="bg-rose-50/60 px-3 py-1.5 rounded-xl border border-rose-100">
                      <span className="text-rose-500 uppercase text-[9px] block">Stock OUT</span>
                      <span className="text-rose-700 font-black">
                        -{(dayRecord.summary?.totalStockOutToday || 0).toLocaleString('en-IN')}
                      </span>
                    </div>
                    <div className="bg-blue-50/60 px-3 py-1.5 rounded-xl border border-blue-100">
                      <span className="text-blue-600 uppercase text-[9px] block">Closing Value</span>
                      <span className="text-blue-800 font-black">
                        ₹{(dayRecord.summary?.closingStockValue || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                      </span>
                    </div>
                  </div>
                </div>
                            ...
                            <table className="table w-full">
                              <thead>
                                <tr className="bg-slate-50 text-slate-500 text-[10px] font-extrabold uppercase tracking-wider border-b border-slate-100">
                                  <th className="py-2.5 px-4 text-left w-2/5">Sub Product</th>
                                  <th className="py-2.5 px-4 text-center w-1/12">Unit</th>
                                  <th className="py-2.5 px-4 text-right">Opening</th>
                                  <th className="py-2.5 px-4 text-right text-emerald-600">+IN</th>
                                  <th className="py-2.5 px-4 text-right text-rose-500">-OUT</th>
                ...
                <div className="bg-[#0b1329] text-white py-3 px-5 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-3 text-xs">
                  <span className="font-heading font-black tracking-wider uppercase">
                    {formatDateDDMMYYYY(dayRecord.ledger_date)} Total Movements
                  </span>
