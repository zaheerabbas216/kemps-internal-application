...
const CanDepositLedger = () => {
  ...
  const openNewDepositModal = () => {
    ...
    setDepSearchText('');
    setDepositForm({ qty: '', rate: '150', particular: 'Can Deposit', paymentMode: 'Cash', remarks: '' });
    setDepositError('');
    setDepositSuccess('');
  ...
  const handleSaveDeposit = async (e) => {
    ...
    try {
      const payload = {
        ...
        remarks: `[Particular: ${depositForm.particular || 'Can Deposit'}] ${depositForm.remarks || ''}`.trim(),
  ...
  const getDisplayRemarks = (remarks) => {
    if (!remarks) return '—';
    const match = remarks.match(/^\[Particular: (.*?)\]\s*(.*)$/);
    if (match) {
  ...
  const handlePrintReceipt = (tx) => {
    ...
    let particularName = 'Can Deposit';
    let displayRemarks = tx.remarks || '';
    if (tx.remarks) {
      const match = tx.remarks.match(/^\[Particular: (.*?)\]\s*(.*)$/);
      if (match) {
        particularName = match[1];
        displayRemarks = match[2];
      }
    }
        ...
        <body>
          ...
          <table class="meta-table">
            ...
            <tr>
              ...
              <td class="right">${particularName}</td>
            </tr>
            <tr>
            ...
            <tr>
              <td class="bold">Particular:</td>
              <td class="right">${particularName} (Refund)</td>
            </tr>
            `}
            <tr>
            ...
            <tr>
              ...
              <td class="right" style="word-break: break-all;">${displayRemarks}</td>
            </tr>
            ` : ''}
            <tr>
  ...
  // Decode selectedTx remarks for details modal
  let viewParticularName = 'Can Deposit';
  let viewDisplayRemarks = '';
  if (selectedTx && selectedTx.remarks) {
    const match = selectedTx.remarks.match(/^\[Particular: (.*?)\]\s*(.*)$/);
    if (match) {
      viewParticularName = match[1];
      viewDisplayRemarks = match[2];
    } else {
      viewDisplayRemarks = selectedTx.remarks;
    }
  }
                ...
                <tbody className="divide-y divide-slate-100 text-slate-700 text-xs font-semibold">
                  {transactions.map(tx => (
                    <tr key={tx.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="py-3.5 px-5 text-slate-400 max-w-[120px] truncate" title={tx.remarks}>
                        {getDisplayRemarks(tx.remarks)}
                      </td>
                      <td className="py-3.5 px-5 text-center text-slate-505">{tx.created_by}</td>
                      <td className="py-3.5 px-5 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          ...
                          {isAdmin ? (
                            <button 
                              ...
                              className="px-2 py-1 rounded bg-red-50 border border-red-100 text-red-550 hover:bg-red-100 hover:text-red-700 font-bold transition-all text-[11px]"
            ...
            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
              <form id="depositForm" onSubmit={handleSaveDeposit} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 border border-slate-150 p-5 rounded-2xl bg-[#fcfdff]/50">
                  ...
                  {/* Particular (Manual Input) */}
                  <div className="space-y-1.5">
                    <label className="label-premium block">Particular</label>
                    <input
                      type="text"
                      placeholder="e.g. Can Deposit"
                      value={depositForm.particular || ''}
                      onChange={(e) => setDepositForm(prev => ({ ...prev, particular: e.target.value }))}
          ...
          <div className="modal-box bg-white border border-slate-200 rounded-3xl p-8 relative shadow-2xl max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="border border-slate-200 p-6 rounded-2xl bg-slate-50/50 space-y-4">
              ...
              <div className="space-y-3.5 text-xs font-semibold text-slate-700">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <span className="text-slate-400 font-bold text-[10px] uppercase">Particular</span>
                  <span className="col-span-2 font-extrabold text-slate-800">{viewParticularName}</span>
                </div>
                {selectedTx.qty > 0 && (
                  <>
                    <div className="grid grid-cols-3 gap-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <span className="col-span-2 font-black text-slate-855 text-sm">₹ {parseFloat(selectedTx.amount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                </div>
                <div className="grid grid-cols-3 gap-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <span className="col-span-2 font-normal text-slate-500 italic whitespace-pre-wrap">{viewDisplayRemarks || '—'}</span>
                </div>
              </div>
            </div>
