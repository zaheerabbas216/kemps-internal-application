...
const Loading = () => {
  ...
  const handleSaveReturn = async (e) => {
    ...
    const selectedItem = loadedItems.find(i => String(i.finishedProductId) === String(returnForm.finishedProductId));
    if (!selectedItem) {
      setReturnError('Please select a valid product.');
      return;
    }
    const maxAvailable = selectedItem.netLoadingQty;
    const qtyNum = parseInt(returnForm.quantity, 10);
    if (isNaN(qtyNum) || qtyNum <= 0) {
      setReturnError('Quantity must be greater than 0.');
      return;
    }
    if (qtyNum > maxAvailable) {
      setReturnError(`Cannot return more than available quantity (${maxAvailable}).`);
      return;
    }

    const finalReason = returnForm.reason === 'Other' 
      ? returnForm.customReason.trim() 
      : returnForm.reason;
    ...
    try {
      ...
      const res = await api.post(`/loading/session/${returnSession.id}/return`, {
        finishedProductId: parseInt(returnForm.finishedProductId, 10),
        quantity: qtyNum,
        reason: finalReason
      });
  ...
  }, [activeTab, historyPage, filterSearch, filterStartDate, filterEndDate, filterGodown]);
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .header h3 { margin: 0; text-transform: uppercase; font-size: 13px; letter-spacing: 0.5px; }
            .header p { margin: 2px 0; font-size: 9px; }
            .meta-table, .items-table { width: 100%; border-collapse: collapse; font-size: 10px; }
            .meta-table td { padding: 1px 0; }
            .items-table th { border-bottom: 1px dashed #000; padding: 4px 0; text-align: left; font-size: 9px; }
            .items-table td { padding: 3px 0; vertical-align: top; }
            .right { text-align: right; }
            .remarks-box { margin-top: 5px; font-size: 9px; font-style: italic; border: 1px dotted #000; padding: 4px; }
            .sig-section { margin-top: 30px; display: flex; justify-content: space-between; font-size: 9px; }
            .sig-box { width: 80px; text-align: center; }
        ...
        <body>
          <div class="header center">
            ...
            <p>QUALITY WATER & JUICE SUPPLIERS</p>
          ...
          <table class="meta-table">
            ...
            <tr>
              ...
              <td class="right">${formatDateDDMMYYYY(printData.trip.loading_date)}</td>
            </tr>
            <tr>
              <td class="bold">Time:</td>
              <td class="right">${new Date(printData.trip.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td class="right">${printData.trip.customer_phone}</td>
            </tr>
          </table>
          ...
          <table class="items-table">
            <thead>
              <tr>
                ...
                <th class="right" style="width: 15%;">RET</th>
                <th class="right" style="width: 15%;">NET</th>
              </tr>
            </thead>
            <tbody>
          ...
          ${printData.trip.remarks ? `
            <div class="remarks-box">
              <span class="bold">Remarks:</span> ${printData.trip.remarks}
            </div>
            <div class="divider"></div>
          ` : ''}
              ...
              {formItems.map((row, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="md:col-span-1.5 space-y-1.5 flex gap-2 items-end">
        ...
        <div className="modal modal-open animate-fade-in z-[60]">
          <div className="modal-box bg-white p-6 flex flex-col items-center justify-center max-w-sm rounded-3xl shadow-2xl relative z-10 max-h-[90vh] overflow-y-auto">
            ...
            <div 
              ...
              className="bg-white text-black font-mono p-5 rounded-xl border border-slate-200 w-80 text-xs flex flex-col pos-slip-container"
            >
              {loadingPrint ? (
                <div className="py-20 flex flex-col items-center justify-center text-slate-500">
                  <span className="loading loading-spinner text-slate-700"></span>
                  <span className="text-[10px] font-bold mt-2 uppercase tracking-widest">Generating Slip...</span>
                </div>
              ) : printData ? (
                <>
                  {/* Shop header info */}
                  <div className="text-center space-y-1 pb-3 border-b border-dashed border-black">
                    <h2 className="text-sm font-black tracking-tighter uppercase font-sans">KEMP'S INDUSTRIES</h2>
                    <p className="text-[9px] font-sans text-slate-500 font-bold">QUALITY WATER & JUICE SUPPLIERS</p>
                    <p className="text-[9px] font-bold">Godown: {printData.trip.godown === 'KI' ? "Kempannavar (KI)" : "Kemps Pet (KP)"}</p>
                  </div>

                  {/* Trip / Loading details header */}
                  <div className="py-3 border-b border-dashed border-black space-y-1 w-full">
                    <div className="flex justify-between">
                      <span>Trip:</span>
                      <span className="font-bold">Trip #${printData.trip.trip_number}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Date:</span>
                      <span>{formatDateDDMMYYYY(printData.trip.loading_date)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Time:</span>
                      <span>{new Date(printData.trip.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
                    <div className="flex justify-between mt-1 pt-1 border-t border-slate-100">
                      <span>Cust Name:</span>
                      <span className="font-bold">{printData.trip.customer_name}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Phone:</span>
                      <span>{printData.trip.customer_phone}</span>
                    </div>
                  </div>

                  {/* Loading Table Products list */}
                  <div className="py-3 border-b border-dashed border-black w-full">
                    <div className="grid grid-cols-12 font-bold mb-1 border-b border-slate-100 pb-1 text-[10px]">
                    ...
                    <div className="space-y-1">
                      {printData.items.map(item => (
                        <div key={item.id} className="grid grid-cols-12 text-[10px]">
                          ...
                          <span className="col-span-2 text-center text-slate-600">{item.returnQty}</span>
                          <span className="col-span-2 text-right font-bold">{item.netLoadingQty}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {/* Remarks details */}
                  {printData.trip.remarks && (
                    <div className="py-2 border-b border-dashed border-black text-[10px] w-full">
                      <span className="font-bold block">Remarks/Notes:</span>
                      <p className="italic text-slate-600 break-words">{printData.trip.remarks}</p>
                    </div>
                  )}

                  {/* Signatures footer */}
                  <div className="mt-8 pt-4 flex justify-between text-[10px] text-center border-t border-slate-100 w-full">
                    <div className="w-24">
                  ...
                  <div className="text-center text-[9px] mt-6 font-bold text-slate-400 font-sans tracking-wide uppercase">
                    *** Thank you ***
                  </div>
                </>
              ) : null}
        ...
        <div className="modal modal-open animate-fade-in z-[60]">
          <div className="modal-box max-w-md bg-white border border-slate-200/80 rounded-3xl p-6 relative shadow-2xl z-10 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSaveReturn} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] font-black text-slate-500 uppercase tracking-wider block">
                  Select Loaded Product *
                </label>
                <select
                  value={returnForm.finishedProductId}
                  onChange={(e) => setReturnForm(prev => ({ ...prev, finishedProductId: e.target.value }))}
                  className="w-full h-11 px-4 rounded-xl border border-slate-200 bg-white text-slate-700 focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all outline-none text-sm font-semibold"
                  required
                >
                  <option value="">-- Choose Product --</option>
                  {(returnSession?.items || returnSession?.consolidatedItems || [])
                    .filter(item => item.quantity > 0)
                    .map(item => (
                      <option key={item.finishedProductId} value={item.finishedProductId}>
                        {item.productName} (Avail: {item.netLoadingQty})
                      </option>
                    ))}
                </select>
              </div>

              {/* Show loaded stats if a product is selected */}
              {returnForm.finishedProductId && (
                (() => {
                  const itemsList = returnSession?.items || returnSession?.consolidatedItems || [];
                  const item = itemsList.find(i => String(i.finishedProductId) === String(returnForm.finishedProductId));
                  if (!item) return null;
                  return (
                    <div className="bg-slate-50 border border-slate-150 p-3.5 rounded-2xl grid grid-cols-3 gap-2 text-center text-xs font-bold text-slate-550">
                      <div>
                        <div className="text-[9px] font-black text-slate-450 uppercase tracking-wider">Loaded</div>
                        <div className="text-slate-800 font-black mt-0.5">{item.quantity}</div>
                      </div>
                      <div>
                        <div className="text-[9px] font-black text-slate-450 uppercase tracking-wider">Returned</div>
                        <div className="text-rose-500 font-black mt-0.5">{item.returnQty}</div>
                      </div>
                      <div>
                        <div className="text-[9px] font-black text-slate-450 uppercase tracking-wider">Available</div>
                        <div className="text-primary font-black mt-0.5">{item.netLoadingQty}</div>
                      </div>
                    </div>
                  );
                })()
              ...
              <div className="space-y-1.5">
                ...
                >
                  ...
                  <option value="Excess Quantity Loaded">Excess Quantity Loaded</option>
                  <option value="Damaged Goods During Loading">Damaged Goods During Loading</option>
                  <option value="Customer Refused Certain Items">Customer Refused Certain Items</option>
              ...
              {returnForm.reason === 'Other' && (
                <div className="space-y-1.5 animate-fade-in">
                  ...
                  <textarea
                    ...
                    onChange={(e) => setReturnForm(prev => ({ ...prev, customReason: e.target.value }))}
                    className="w-full p-3 rounded-xl border border-slate-200 bg-white text-slate-755 focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all outline-none text-xs font-medium h-20 resize-none"
                    required
                  />
                </div>
              )}
              ...
              {returnError && (
                <div className="bg-rose-50 text-rose-600 px-4 py-2.5 rounded-xl text-[11px] font-semibold border border-rose-100">
                  ⚠️ {returnError}
                </div>
              )}
