...
const Inventory = () => {
  ...
  const [summary, setSummary] = useState({ totalValue: 0, totalCount: 0 });
  ...
  // Bill Items Form State (Dynamic array of products)
  ...
  const fetchTodayInventory = async () => {
    try {
      ...
      if (res.data.ok) {
        ...
        setSummary(res.data.summary || { totalValue: 0, totalCount: 0 });
        if (res.data.todayStr) {
          setTodayDateStr(res.data.todayStr);
        }
      }
    } catch (err) {
      console.error('Failed to fetch today\'s inventory:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenForm = async (bill = null) => {
    setFormError('');
    setFormSuccess('');
    ...
    if (bill) {
      // Edit mode (optional logic, fetching details)
      try {
        setLoading(true);
        ...
        if (res.data.ok) {
          ...
          setBillHeader({
            billDate: res.data.bill.bill_date ? res.data.bill.bill_date.split('T')[0] : todayFormatted,
      ...
      // Start with 1 empty product row
      setBillItems([createNewItemRow()]);
    }
    setIsFormOpen(true);
  };
  ...
  const handleAddProductRow = () => {
    setBillItems(prev => [...prev, createNewItemRow()]);
  };

  const handleRemoveProductRow = (index) => {
    if (billItems.length === 1) return; // keep at least one
  ...
  const handleSaveBill = async (e) => {
    ...
    if (!supplierId) return setFormError('Supplier / Company is required.');
    ...
    for (let i = 0; i < billItems.length; i++) {
      ...
      if (!item.rawMaterialId) {
        return setFormError(`Product is missing in Row #${i + 1}.`);
      }
      if (parseFloat(item.totalQuantity) <= 0 || isNaN(parseFloat(item.totalQuantity))) {
        return setFormError(`Total Quantity must be greater than 0 in Row #${i + 1}.`);
      }
      if (parseFloat(item.ratePerUnit) < 0 || isNaN(parseFloat(item.ratePerUnit))) {
        return setFormError(`Rate Per Unit cannot be negative in Row #${i + 1}.`);
      }
      if (isNaN(parseFloat(item.amount)) || parseFloat(item.amount) < 0) {
        return setFormError(`Amount cannot be negative in Row #${i + 1}.`);
      }
    }
    ...
    try {
      let res;
      if (editingBill) {
        res = await api.put(`/inventory/${editingBill.id}`, payload);
      } else {
        res = await api.post('/inventory', payload);
      }

      if (res.data.ok) {
        setFormSuccess(editingBill ? 'Inventory Purchase Bill updated!' : 'Inventory Purchase Bill saved successfully!');
        setTimeout(() => {
          handleCloseForm();
          fetchTodayInventory();
        }, 1000);
      } else {
        setFormError(res.data.error || 'Failed to save inventory bill.');
      }
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.data.ok) {
        ...
        alert(res.data.error || 'Failed to delete purchase record.');
      }
    } catch (err) {
  ...
  const handleOpenView = async (bill) => {
    try {
      ...
      alert('Failed to load bill details for viewing.');
    } finally {
      setLoading(false);
    }
  };
  ...
  // PDF Print single bill register invoice
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .meta-grid { display: grid; grid-cols: 2; gap: 20px; font-size: 13px; margin-bottom: 30px; }
    ...
    <>
      {isFormOpen ? (
        <div className="space-y-6 animate-fade-in max-w-4xl mx-auto pb-12">
          ...
          {/* Heading */}
          <div>
            <h1 className="text-2xl font-black text-slate-800 tracking-tight uppercase">INVENTORY INPUT</h1>
            <p className="text-slate-500 text-xs font-semibold mt-1">Select company first, then add products — one bill for all products</p>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="border border-slate-200/80 rounded-2xl bg-white p-5 space-y-5 shadow-sm">
              ...
              <p className="text-xs text-slate-400 italic">All products below will be saved under the same company bill.</p>
                ...
                {billItems.map((item, index) => {
                  ...
                  return (
                    ...
                    <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                      ...
                      {billItems.length > 1 && (
                        <button
                          ...
                          className="w-7 h-7 rounded-full bg-slate-50 border border-slate-200 text-slate-500 flex items-center justify-center hover:bg-red-50 hover:text-red-500 hover:border-red-200 transition-all font-bold text-xs"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      {/* Total Amount (Auto) */}
                      <div className="space-y-1.5">
                        <label className="text-[12px] font-bold text-slate-400 block uppercase tracking-wider">
                          Total Amount (Auto)
                        </label>
                        <input
                          type="text"
                          ...
                          readOnly
                          className="w-full h-11 px-4 rounded-xl border border-slate-200 bg-blue-50/20 text-slate-700 outline-none text-sm font-bold"
                        />
                      </div>
              ...
              <button
                ...
                className="w-full py-4 border-2 border-dashed border-blue-200 rounded-2xl text-blue-600 bg-blue-50/15 hover:bg-blue-50/30 font-bold text-xs uppercase tracking-wider transition-all duration-200 flex items-center justify-center gap-1.5"
              >
                + Add Another Product
              </button>
            </div>
            ...
            {/* BILL SUMMARY SECTION FOOTER (Navy Box matching the exact layout of the image) */}
            <div className="bg-[#0b1324] rounded-2xl p-5 text-white flex flex-col md:flex-row items-center justify-between gap-4 mt-6">
              <div className="flex-1 text-center">
                <span className="text-[10px] font-bold text-slate-450 uppercase tracking-widest block">PRODUCTS</span>
                <span className="text-xl font-extrabold">{summaryData.productsCount}</span>
              </div>
              <div className="hidden md:block w-[1px] h-8 bg-slate-800"></div>
              <div className="flex-1 text-center">
                <span className="text-[10px] font-bold text-slate-450 uppercase tracking-widest block">SUB TOTAL</span>
                <span className="text-xl font-extrabold">₹{summaryData.subTotal.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
              </div>
              <div className="hidden md:block w-[1px] h-8 bg-slate-800"></div>
              <div className="flex-1 text-center">
                <span className="text-[10px] font-bold text-slate-450 uppercase tracking-widest block">TOTAL TAX</span>
                <span className="text-xl font-extrabold">₹{summaryData.totalTax.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
              </div>
              <div className="hidden md:block w-[1px] h-8 bg-slate-800"></div>
              <div className="flex-1 text-center">
                <span className="text-[10px] font-bold text-slate-450 uppercase tracking-widest block">GRAND TOTAL</span>
                <span className="text-xl font-black text-sky-400">₹{summaryData.grandTotal.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
              </div>
            </div>

            {/* Form action save green button */}
            <button
              type="button"
              onClick={handleSaveBill}
              disabled={isSaving}
              className="w-full bg-[#10b981] hover:bg-[#059669] text-white h-14 rounded-2xl font-bold uppercase tracking-wider transition-all duration-200 flex items-center justify-center gap-2 shadow-lg shadow-emerald-100/30"
            >
              {isSaving ? (
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl font-black text-slate-800 tracking-tight">INVENTORY INPUT</h1>
              <p className="text-slate-500 text-sm font-medium mt-1">Record raw material purchases and supplier bills.</p>
            </div>
            <div className="flex items-center gap-3">
              ...
              >
                <span className="text-xl">+</span> Add Inventory Bill
              </button>
              <button 
          ...
          {/* TODAY'S AGGREGATES SUMMARY CARDS */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="card-premium flex items-center justify-between">
              <div>
                <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Today's Total Purchases</p>
            ...
            <div className="card-premium flex items-center justify-between">
              <div>
                <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Today's Total Bills</p>
                <h3 className="text-2xl font-extrabold text-[#0f172a] mt-1.5">
                  {summary.totalCount} {summary.totalCount === 1 ? 'Bill' : 'Bills'}
                </h3>
              </div>
          ...
          <div className="card-premium">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
              <div className="relative flex-1 max-w-md">
                ...
                <input 
                  type="text" 
                  placeholder="Search by bill number, supplier, product..."
                  value={searchQuery}
              ...
              <div className="text-[12px] font-bold text-slate-400 uppercase tracking-widest bg-slate-50 px-3.5 py-1.5 rounded-lg border border-slate-100">
                Today's Inventory Entries — {todayDateStr ? formatDateDDMMYYYY(todayDateStr) : formatDateDDMMYYYY(new Date().toISOString().split('T')[0])}
              </div>
            </div>
                  ...
                  {loading ? (
                    <tr>
                      <td colSpan="8" className="py-20 text-center">
                        <div className="flex flex-col items-center gap-3">
                           ...
                           <span className="text-slate-400 text-sm font-medium">Fetching today's purchases...</span>
                        </div>
                      </td>
                    </tr>
                  ) : bills.length === 0 ? (
                    <tr>
                      <td colSpan="8" className="py-20 text-center text-slate-400 font-medium italic">
                        {searchQuery ? 'No matching purchase bills found for today.' : 'No purchase bills recorded today.'}
                      </td>
                    </tr>
                  ) : (
                    bills.map((bill) => (
                      <tr key={bill.id} className="hover:bg-blue-50/30 transition-colors group">
                        ...
                        <td className="py-4 px-6 text-[13px] font-medium text-slate-550">{bill.billed_to}</td>
                        <td className="py-4 px-6 text-[13px] text-slate-500 max-w-[200px] truncate" title={bill.products_summary}>
                        ...
                        <td className="py-4 px-6 text-[13px] font-semibold text-slate-605 uppercase">{bill.payment_method}</td>
          ...
          <div className="bg-white rounded-3xl shadow-[0_25px_80px_rgba(0,0,0,0.2)] border border-slate-200 w-[720px] h-[80vh] flex flex-col overflow-hidden animate-fade-in pointer-events-auto">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-slate-50/50">
              ...
              <div className="border border-slate-200/80 rounded-2xl bg-white overflow-hidden shadow-sm">
                <div className="bg-slate-50/50 px-5 py-3 border-b border-slate-100 text-[10px] font-black text-slate-500 uppercase tracking-widest">
                ...
                <div className="p-4 space-y-4">
                  {viewingBill.items.map((item, idx) => (
                    <div key={item.id} className="border border-slate-100 rounded-xl p-4 text-xs font-semibold text-slate-650 bg-[#fafcff]/50 space-y-2">
                      ...
                      {item.remarks && (
                        <div className="pt-1.5 border-t border-slate-100 mt-1.5 text-slate-400 italic">
                          Remarks: {item.remarks}
                        </div>
                      )}
                    </div>
