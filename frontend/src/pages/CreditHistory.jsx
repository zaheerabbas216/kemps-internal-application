...
const CreditHistory = () => {
  ...
  const handlePrintSinglePayment = (p) => {
    const printWindow = window.open('', '_blank');
    const html = `
      <html>
        <head>
          <title>Credit Payment Receipt — ${p.payment_id}</title>
          <style>
            body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; padding: 25px; color: #1e293b; }
            .header { text-align: center; border-bottom: 2px solid #0b1324; padding-bottom: 12px; margin-bottom: 20px; }
            .header h2 { margin: 0; color: #0b1324; text-transform: uppercase; font-size: 18px; letter-spacing: 1px; }
            .header p { margin: 4px 0 0 0; color: #64748b; font-size: 12px; font-weight: bold; }
            .box { border: 1px solid #cbd5e1; padding: 15px; border-radius: 10px; margin-bottom: 15px; background: #f8fafc; }
            .row { display: flex; justify-content: space-between; margin-bottom: 8px; font-size: 13px; }
            .label { font-weight: bold; color: #475569; }
            .val { font-weight: bold; color: #0f172a; }
            .amount { font-size: 18px; font-weight: 900; color: #16a34a; text-align: right; }
            .footer { margin-top: 30px; text-align: center; font-size: 11px; color: #94a3b8; border-top: 1px solid #e2e8f0; padding-top: 10px; }
          </style>
        </head>
        <body>
          <div class="header">
            <h2>Kemp's Beverages & Bottling Plants</h2>
            <p>Official Credit Payment Receipt</p>
          </div>
          <div class="box">
            <div class="row"><span class="label">Payment Receipt ID:</span><span class="val">${p.payment_id}</span></div>
            <div class="row"><span class="label">Payment Date:</span><span class="val">${formatDateDDMMYYYY(p.payment_date)}</span></div>
            <div class="row"><span class="label">Invoice Number:</span><span class="val">${p.invoice_no}</span></div>
            <div class="row"><span class="label">Customer Name:</span><span class="val">${p.customer_name}</span></div>
            <div class="row"><span class="label">Company:</span><span class="val">${p.company}</span></div>
            <div class="row"><span class="label">Customer Type:</span><span class="val">${p.customer_type}</span></div>
            <div class="row"><span class="label">Payment Method:</span><span class="val">${p.payment_method}</span></div>
          </div>
          <div class="box" style="background: #ecfdf5; border-color: #a7f3d0;">
            <div class="row"><span class="label">Invoice Total:</span><span class="val">₹ ${parseFloat(p.invoice_amount).toFixed(2)}</span></div>
            <div class="row"><span class="label">Amount Received:</span><span class="val amount">₹ ${parseFloat(p.amount_received).toFixed(2)}</span></div>
          ${p.notes ? `<div class="box"><span class="label">Notes / Remarks:</span> <span class="val">${p.notes}</span></div>` : ''}
          <div class="footer">Thank you for your payment. This is a computer generated payment receipt.</div>
          <script>
  ...
  const getPaymentBreakdown = (p) => {
    if (!p) return { cash: 0, upi: 0, bank: 0 };
    const cash = parseFloat(p.cash_paid || 0);
    const upi  = parseFloat(p.upi_paid || 0);
    const bank = parseFloat(p.bank_paid || 0);
    const total = parseFloat(p.amount_received || 0);

    if (cash > 0 || upi > 0 || bank > 0) {
      return { cash, upi, bank };
    }

    const meth = String(p.payment_method || '').toUpperCase();
    const hasCash = meth.includes('CASH');
    const hasUpi  = meth.includes('UPI');
    const hasBank = meth.includes('BANK');

    const count = [hasCash, hasUpi, hasBank].filter(Boolean).length;
    if (count > 0) {
      const perMethod = total / count;
      return {
        cash: hasCash ? perMethod : 0,
        upi:  hasUpi  ? perMethod : 0,
        bank: hasBank ? perMethod : 0
      };
    }

    return { cash: total, upi: 0, bank: 0 };
  };
    ...
    <div className="space-y-6 animate-fade-in max-w-5xl mx-auto pb-12">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/75 border-b border-slate-200/80 text-[10px] font-black text-slate-450 uppercase tracking-wider">
                    ...
                    <th className="py-4 px-4 text-right">Total Received</th>
                ...
                <tbody className="divide-y divide-slate-100 text-slate-750 text-xs font-semibold">
                  {payments.map(p => (
                    <tr key={p.payment_id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="py-3.5 px-4 font-mono text-[11px] text-slate-400">{p.payment_id}</td>
                      <td className="py-3.5 px-4">{formatDateDDMMYYYY(p.payment_date)}</td>
                      <td className="py-3.5 px-4 font-bold text-primary">{p.invoice_no}</td>
                      ...
                      <td className="py-3.5 px-4 uppercase text-[9px] tracking-wider text-slate-500 font-bold">
                        {p.customer_type}
                      </td>
                      ...
                      <td className="py-3.5 px-4 text-center text-slate-550 font-bold uppercase text-[10px]">
                        {p.payment_method}
                      </td>
                      ...
                      <td className="py-3.5 px-4 text-center">
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-primary/10 text-primary hover:bg-primary hover:text-white transition-all font-bold text-xs flex items-center justify-center gap-1 mx-auto shadow-xs"
                        >
                          👁️ View
                        </button>
                      </td>
                  ...
                  {payments.length === 0 && (
                    <tr>
                      <td colSpan="11" className="py-12 text-center text-slate-400 font-semibold text-xs bg-slate-50/20">
      ...
      {selectedPayment && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm pointer-events-auto">
          <div className="bg-white rounded-3xl shadow-[0_25px_80px_rgba(0,0,0,0.2)] border border-slate-200 w-[540px] overflow-hidden animate-fade-in pointer-events-auto">
            {/* Header */}
            <div className="bg-[#0b1324] p-5 text-white flex items-center justify-between">
              <div>
                <span className="text-[10px] font-black text-emerald-400 uppercase tracking-widest bg-emerald-950/80 px-2 py-0.5 rounded border border-emerald-500/30">
            ...
            <div className="p-6 space-y-4 text-xs font-semibold text-slate-600">
              <div className="grid grid-cols-2 gap-3 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                <div>
                  ...
                  <p className="text-sm font-black text-slate-800">{selectedPayment.customer_name}</p>
                  {selectedPayment.customer_phone && (
                    <p className="text-[11px] text-slate-500 font-medium">{selectedPayment.customer_phone}</p>
                  )}
                </div>
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase">Company &amp; Type</p>
                  <p className="text-xs font-bold text-primary">{selectedPayment.company}</p>
                  <p className="text-[11px] text-slate-500 uppercase">{selectedPayment.customer_type}</p>
                </div>
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase">Payment Date</p>
                  <p className="text-xs font-bold text-slate-800">{formatDateDDMMYYYY(selectedPayment.payment_date)}</p>
                </div>
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase">Invoice Number</p>
                  <p className="text-xs font-mono font-bold text-slate-800">{selectedPayment.invoice_no}</p>
                </div>
              </div>
              ...
              <div className="bg-emerald-50/60 border border-emerald-200/80 p-4 rounded-2xl space-y-2">
                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-600 font-bold">Invoice Grand Total:</span>
                  <span className="font-bold text-slate-800">{formatINR(selectedPayment.invoice_amount)}</span>
                </div>
                <div className="flex justify-between items-center text-sm font-black border-t border-emerald-200/60 pt-2 text-emerald-800">
                  <span>Amount Received Now:</span>
                  <span>{formatINR(selectedPayment.amount_received)}</span>
                </div>
                ...
                {(() => {
                  ...
                  return (
                    <div className="grid grid-cols-3 gap-2 pt-2 text-[11px] font-bold border-t border-emerald-200/40 text-slate-600">
                      <div>Cash Paid: <strong className="text-emerald-700">{formatINR(bd.cash)}</strong></div>
                      <div>UPI Paid: <strong className="text-indigo-700">{formatINR(bd.upi)}</strong></div>
                      <div>Bank Paid: <strong className="text-sky-700">{formatINR(bd.bank)}</strong></div>
                    </div>
                  );
                })()}

                <div className="flex justify-between items-center text-xs font-bold border-t border-emerald-200/60 pt-2 text-amber-800">
                  <span>Remaining Credit Due:</span>
                  <span>{formatINR(selectedPayment.due_amount)}</span>
                </div>
              </div>

              {selectedPayment.notes && (
                <div className="p-3 bg-slate-50 border border-slate-200/60 rounded-xl text-slate-600 text-xs">
                  <span className="font-bold uppercase text-[10px] text-slate-400 block mb-0.5">Notes / Remarks</span>
                  {selectedPayment.notes}
                </div>
              )}
            </div>
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
              <button
                type="button"
                onClick={() => setSelectedPayment(null)}
                className="px-5 h-10 rounded-xl border border-slate-200 text-slate-600 bg-white hover:bg-slate-50 font-bold text-xs"
              >
                Close
              </button>
              <button
                type="button"
                onClick={() => handlePrintSinglePayment(selectedPayment)}
                className="btn-premium bg-slate-900 text-white hover:bg-slate-800 text-xs h-10 px-5 font-bold flex items-center gap-1.5"
              >
                🖨️ Print Receipt
              </button>
            </div>
