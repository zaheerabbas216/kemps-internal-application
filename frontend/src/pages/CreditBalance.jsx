...
const CreditBalance = () => {
  ...
  const handleOpenPay = (bill) => {
    ...
    setPaymentForm({
      ...
      paymentMethod: 'Cash',
      amountReceived: String(bill.due_amount), // prefill with full outstanding balance
      remarks: ''
    });
  ...
  const handleSavePayment = async (e) => {
    ...
    if (isNaN(amt) || amt <= 0) {
      return setPayError('Amount received must be greater than 0.');
    }
    if (amt > parseFloat(selectedBill.due_amount)) {
      return setPayError(`Amount cannot exceed the remaining outstanding balance of ₹${parseFloat(selectedBill.due_amount).toFixed(2)}.`);
    }
            ...
            <form onSubmit={handleSavePayment} className="space-y-4 mt-6">
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  required
                >
                  {['Cash', 'Bank Transfer', 'UPI', 'Cheque', 'Credit Adjustment'].map(method => (
